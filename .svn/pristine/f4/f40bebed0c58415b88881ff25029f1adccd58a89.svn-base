...
var config = {
	...
	api_detail: api_url + '/supplierContract/editInfo', // 获取合同详情
	api_edit: api_url + '/supplierContract/saveOrupdate', // 修改合同
	api_supplierList: api_url + '/supplier/supplierList', //获取供应商列表
	api_saveProductAttach: api_url + '/supplier/addAttach',
	api_delAttach: api_url + '/supplierContract/delAttach', //删除
}
			...
			$.ajax({
				...
				success: function(res) {
					...
					if(res.error == "00") {
						...
						that.signingTime = that.list.signingTime.substr(0, 10);
						that.endTime = that.list.endTime.substr(0, 10);;
						that.remark = that.list.remark;
						that.supplierId = that.list.supplierId;
						that.c.attachs = that.list.attachList
					} else {
						layer.msg(res.msg)
					}
				}
