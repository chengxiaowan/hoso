...
require(['select2'], (select2) => {
	window.app = new Vue({
		...
		data: {
			...
			dxmonth2: new Date().getFullYear().toString(),
			dxmonth3: new Date().getFullYear().toString(),
			dxmonth4: new Date().getFullYear().toString(),
			dx: {
				dkje: '',
				mbdkje: '',
				htsl: '',
		...
		mounted() {
			...
			this.getYjmb(that.dxmonth, 0); //商务顾问
		...
		methods: {
			...
			dxhx(hhd, hhd1, time, gxtime) {
				...
				pieChart.on("click", function(params) {
					var html = '/static/page/index/saler_turnover_list.html?year=' + time + '&gxtime=' + gxtime1 + '&USER_ID=' + that.saleId + '&flag=3'+'&htz=1';
