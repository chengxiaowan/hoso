var config = {
    ...
    api_save: api_url + '/service/addPhotoService',
...
window.app = new Vue({
    ...
    methods: {
        ...
        save() {
            ...
            if (that.name =='') {
                ...
                $.ajax({
                    ...
                    success: res => {
                        if(res.error == '00'){
                            layer.msg("保存成功,清新增旅拍日期",{time:3000})
                            console.log(res.result.split(','))
                            for(var i in res.result.split(',')){
                                sessionStorage.setItem('id',res.result.split(',')[0] );
                                sessionStorage.setItem('type',res.result.split(',')[1] );
                            }
                        }else{
                          layer.msg(res.msg)
                        }
        ...
        search(){
            ...
            that.getData(page,startTime,endTime);
        },
    }
})
