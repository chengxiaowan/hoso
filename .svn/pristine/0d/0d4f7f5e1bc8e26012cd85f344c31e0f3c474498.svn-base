var config = {
	...
	api_list: api_url + '/stock/stockOutList', //获取列表
	api_del: api_url + '/stock/delStockOut', //删除库存入库单
	api_depotList2: api_url + '/depot/depotList2', //获取仓库
	api_auditStockIn: api_url + '/stock/auditStockOut', //审核
}
window.app = new Vue({
	el: '#app',
	data: {
	...
	methods: {
		...
		edit(id) {
			var index = layer.open({
				type: 2,
				title: '编辑出库单',
				content: 'stockOutEdit.html?id=' + id,
				area: ['100%', '100%']
			});
		},
		// 入库单详情
		detail(id) {
			var index = layer.open({
				type: 2,
				title: '出库单详情',
				content: 'stockOutDetail.html?id=' + id,
				area: ['100%', '100%']
			});
		},
		audit(id) {
		...
		del(id) {
			const that = this;
			const dialog = layer.confirm("确认删除该出库单?", {
				title: "提示"
			}, () => {
				$.get(config.api_del, {
					stockOutId: id
