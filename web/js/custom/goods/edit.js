var goodsId = parameter().id;
var config = {
	role: localStorage.userRole,
	api_detail: api_url + '/goods/editInfo', // 获取商品详情
	...
	api_editDinfo: api_url + '/goods/editDescriptionInfo', //获取描述
...
//					}
			...
			$.ajax({
				...
				success: function(res) {
					...
					if(res.error == "00") {
						...
						that.stockNum = that.list.stockNum
						that.stockType = that.list.stockType
		...
		getAttributeList: function(page) {
			...
			if(page) this.attrsList.pageNum = page
			var that = this;
			that.loading();
			$.ajax({
				...
				data: {
					goodsId: that.goodsId,
					pageSize: that.attrsList.pageSize || 10,
					pageNo: that.attrsList.pageNum || 1,
				},
				success: function(res) {
					that.loading('close')
					if(res.error == "00") {
						that.attrsList = res.result;
						//分页
						if(that.pagi) {
							$('.pagi').pagination('updatePages', that.attrsList.pages)
							if(page == 1) $('.pagi').pagination('goToPage', that.attrsList.pageNum)
						} else {
							that.pagi = $('.pagi').pagination({
								pages: that.attrsList.pages, //总页数
								showCtrl: true,
								displayPage: 6,
								currentPage: that.attrsList.pageNum,
								onSelect: function(num) {
									that.attrsList.pageNum = num
									that.getAttributeList();
									yo.scrollTo('body')
								}
							})
		...
		getColorList: function(page) {
			$('body,html').scrollTop(0)
			if(page) this.colorsList.pageNum = page
			var that = this;
			that.loading();
			$.ajax({
				...
				data: {
					goodsId: that.goodsId,
					pageSize: that.colorsList.pageSize || 10,
					pageNo: that.colorsList.pageNum || 1,
				},
				success: function(res) {
					that.loading('close')
					if(res.error == "00") {
						that.colorsList = res.result;
						that.colorList = res.result.list;
						//分页
						if(that.pagi) {
							$('.pagi').pagination('updatePages', that.colorsList.pages)
							if(page == 1) $('.pagi').pagination('goToPage', that.colorsList.pageNum)
						} else {
							that.pagi = $('.pagi').pagination({
								pages: that.colorsList.pages, //总页数
								showCtrl: true,
								displayPage: 6,
								currentPage: that.colorsList.pageNum,
								onSelect: function(num) {
									that.colorsList.pageNum = num
									that.getColorList()
									yo.scrollTo('body')
								}
							})
		...
		getModelList(page) {
			...
			$.ajax({
				...
				success: function(res) {
					that.loading('close')
					if(res.error == "00") {
						that.modelsList = res.result;
						that.modelList = res.result.list;
						//分页
						if(that.pagi) {
							$('.pagi').pagination('updatePages', that.modelsList.pages)
							if(page == 1) $('.pagi').pagination('goToPage', that.modelsList.pageNum)
						} else {
							that.pagi = $('.pagi').pagination({
								pages: that.modelsList.pages, //总页数
								showCtrl: true,
								displayPage: 6,
								currentPage: that.modelsList.pageNum,
								onSelect: function(num) {
									that.modelsList.pageNum = num
									that.getModelList()
									yo.scrollTo('body')
								}
							})
