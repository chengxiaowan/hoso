var config = {
	...
	api_list: api_url+'/todayRecommend/dataList', //获取轮播图列表
	api_del: api_url + '/todayRecommend/del', //删除品牌
}
window.app = new Vue({
	el: '#app',
	data: {
	...
	methods: {
		...
		edit(id,label) {
			var index = layer.open({
				type: 2,
				title: '编辑',
				content: 'editRecommand.html?id='+id + '&label=' + label,
				area: ['100%', '100%']
			});
		},
		// 搜索
		...
		del(id) {
			const that = this;
			const dialog = layer.confirm("确认删除该特惠商品?", {
				title: "提示"
			}, () => {
				$.get(config.api_del, {
					todayRecommendId: id,
