var config = {
	...
	id: parameter().id,
	api_add: api_url + '/stock/editStockOut', //编辑入库,
	...
	api_attributeList: api_url + '/stock/depotPlaceGoodsSkuList', //获取库存商品sku列表
	api_edit: api_url + '/stock/editStockOutInfo', //获取详情
	api_delSceneGoodsSku: api_url + '/stock/delStockOutGoodsSku', //删除子商品
	api_depotList2: api_url + '/depot/depotList2', //获取仓库
	api_checkStockOutWithGoodsSkuIds: api_url + '/stock/checkStockOutWithGoodsSkuIds', //批量获取商品sku可用库存
}
	...
	methods: {
		...
		getDetail() {
			...
			$.ajax({
				...
				async: true,
				data: {
					stockOutId: config.id
				},
				success: function(res) {
					that.loading('close')
					if(res.error == '00') {
						res.result.stockOutGoodsSkuList = JSON.parse(JSON.stringify(res.result.stockOutGoodsSkuList).replace(/stockOutGoodsSkuList/g, "goodsSkuList"))
						that.goodList = res.result.stockOutGoodsSkuList
						that.stockInNo = res.result.stockOutNo
						that.stockInDate = res.result.stockOutDate
						that.depotId = res.result.depotId
						that.remark = res.result.remark
						that.stockInId= res.result.stockOutId
		...
		save() {
			...
			that.goodList.forEach(function(v) {
				var obj = {
					...
					num: v.num,
					sumB: v.sumB,
					stockOutGoodsSkuId:v.stockOutGoodsSkuId
				};
				arr.push(obj)
			})
			if(that.stockInDate == "") {
		...
		addGroup(arr) {
			...
			$.ajax({
				...
				type: 'post',
				data: {
					stockOutGoodsSkuList: JSON.stringify(arr),
					stockOutNo: that.stockInNo,
					stockOutDate: that.stockInDate,
					depotId: that.depotId,
					remark: that.remark,
					stockOutId:that.stockInId
				},
				success: function(res) {
					that.loading('close')
					if(res.error == "00") {
						...
						$.ajax({
							type:"post",
							url:config.api_checkStockOutWithGoodsSkuIds,
							async:true,
							data:{
								depotId:that.depotId,
								goodsSkuIds:that.skipGoodsIds.join(',')
							},
							success:function(data){
								that.goodList = data.result.depotPlaceGoodsSkuList
							}
						});
					} else {
						layer.msg(res.msg)
