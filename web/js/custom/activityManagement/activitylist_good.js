var config = {
	...
	api_list: api_url + '/activity/activityGroupShoppingGoodsList', //获取拼团商品列表
	...
	api_add: api_url + '/activity/addActivityGroupShoppingGoods', //新增拼团商品
	api_del: api_url + '/activity/delActivityGroupShoppingGoods', //删除拼团商品
	api_check: api_url + '/activity/checkActivityGroupShoppingGoods', //检验是否加入
	api_editInfo: api_url + '/activity/editActivityGroupShoppingGoodsInfo', //修改时获取信息
	api_edit: api_url + '/activity/editActivityGroupShoppingGoods', //修改
	id: parameter().id,
	startTime: parameter().startTime,
	endTime: parameter().endTime,
	api_open: api_url + '/activity/changeActivityGroupShoppingGoodsIsOpen', //是否开启拼团
}
...
var vueApp = new Vue({
	...
	methods: {
										...
										yes() {
											...
											$.ajax({
												...
												data: {
													...
													activityGroupShoppingGoodsSkuList: JSON.stringify(arr),
												},
												success: function(res) {
													that.loading('close')
													if(res.error == '00') {
		...
		del(id) {
			let that = this;
			const dialog = layer.confirm("确认删除该拼团?", {
				title: "提示"
			}, () => {
				$.get(config.api_del, {
					activityGroupShoppingGoodsId: id,
				}, function(data) { // 回调函数
					if(data.error == '00') {
						layer.close(dialog)
						layer.msg("删除成功")
		...
		setUp(id) {
			...
			$.ajax({
				...
				success(res) {
					if(res.error == '00') {
						that.skuList = res.result.activityGroupShoppingGoodsSkuList;
						that.isOpen = res.result.isOpen;
						that.num = res.result.num
		...
		down(id,isOpen){
			...
			const dialog = layer.confirm(name, {
				title: "提示"
			}, () => {
				$.get(config.api_open, {
					activityGroupShoppingGoodsSkuId: id,
					isOpen:isOpen
				}, function(data) { // 回调函数
					if(data.error == '00') {
						layer.close(dialog)
		...
		pintuan(id){
			var index = layer.open({
				type: 2,
				title: '拼团记录',
				content: 'activitylist_good_record.html?id=' + id + "&activityId="+config.id,
				area: ['100%', '100%']
			});
		}
	}
