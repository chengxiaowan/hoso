var config = {
	...
	api_list: api_url + '/activity/activityDateTimeGoodsList', //获取限时购活动日期时间段与商品关系列表
	...
	api_add: api_url + '/activity/addActivityDateTimeGoods', //新增活动商品
	api_del: api_url + '/activity/delActivityDateTimeGoods', //删除爆款商品
	api_check: api_url + '/activity/checkActivityDateTimeGoods', //检验是否加入
	api_editInfo: api_url + '/activity/editActivityDateTimeGoodsInfo', //修改时获取信息
	...
	methods: {
									...
									const dialog = layer.open({
										...
										yes() {
											...
											arr.forEach(function(v) {
												sz++
												if(v.activityMoney == '' || v.stockNum == '' || v.stockNum == undefined || v.activityMoney == undefined) {
													return;
												} else {
													that.isgo = true
													sz1++
											...
											$.ajax({
												...
												data: {
													...
													activityDateTimeGoodsSkuList: JSON.stringify(arr),
												},
												success: function(res) {
													that.loading('close')
													if(res.error == '00') {
		...
		setUp(id) {
			...
			$.ajax({
				...
				success(res) {
					if(res.error == '00') {
						that.skuList = res.result.activityDateTimeGoodsSkuList;
						const dialog = layer.open({
							type: 1,
							title: '加入商品',
							closeBtn: 1,
							...
							yes() {
								...
								that.skuList.forEach(function(i) {
									var obj = {
										activityDateTimeGoodsSkuId:i.activityDateTimeGoodsSkuId,
										goodsSkuId: i.goodsSkuId,
										activityMoney: i.activityMoney,
										stockNum:i.stockNum
									}
